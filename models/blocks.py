...
from torch import (cat as torch_cat, zeros_like as torch_zeros_like, max as torch_max, matmul as torch_matmul,
                   zeros as torch_zeros, float32 as torch_float32, rand as torch_rand, randint as torch_randint)
from torch.nn import (Module as torch_nn_Module, Conv2d as torch_nn_Conv2d, BatchNorm2d as torch_nn_BatchNorm2d, 
                      LeakyReLU as torch_nn_LeakyReLU, Identity as torch_nn_Identity, BatchNorm1d as torch_nn_BatchNorm1d, 
                      Linear as torch_nn_Linear)
...
#     """
#     return torch_exp(-sq_r / (2 * sig**2 + eps))
...
def closest_pool(x, inds):
    ...
    if x.dim() == 1:
        x = x.unsqueeze(0)
    x = torch_cat((x, torch_zeros_like(x[:1, :])), 0)
...
def max_pool(x, inds):
    ...
    max_features, _ = torch_max(pool_features, 1)
    return max_features
...
#         averaged_features.append(torch_mean(x[i0:i0 + length], dim=0))

#         # Increment for next cloud
#         i0 += length

#     # Average features in each batch
#     return torch_stack(averaged_features)
...
def get_graph_feature(x, idx, feat_mode='none'):
    ...
    if feat_mode == 'asym':
        ...
        feature = torch_cat((feature-x1, x1), dim=2).contiguous() # (n_queries, k, num_dims*2)
...
class AdaptiveConv(torch_nn_Module):
    def __init__(self, in_channels, feat_channels, out_channels, bn_momentum, feat_mode, strided=False):
        ...
        self.conv0 = torch_nn_Conv2d(self.feat_channels, self.out_channels, kernel_size=1, bias=False)
        self.conv1 = torch_nn_Conv2d(self.out_channels, self.out_channels*self.in_channels, kernel_size=1, bias=False)
        self.bn0 = torch_nn_BatchNorm2d(self.out_channels, momentum=bn_momentum)
        self.bn1 = torch_nn_BatchNorm2d(self.out_channels, momentum=bn_momentum)
        self.leaky_relu = torch_nn_LeakyReLU(negative_slope=0.1)
    ...
    def forward(self, q_points, s_points, neighb_inds, feat):
        ...
        s_points = torch_cat((s_points, torch_zeros_like(s_points[:1, :])), 0) # (n_supports+1, 3)
        # Add a zero feature for shadow neighbors
        if feat.dim() == 1:
            feat = feat.unsqueeze(0)
        feat = torch_cat((feat, torch_zeros_like(feat[:1, :])), 0) # (n_supports+1, feat_channels)
        ...
        if 'xyz' in self.feat_mode:
            ...
            if 'xyz2' in self.feat_mode and not self.strided:
                ...
                x = torch_cat((x-q_points, q_points), dim=2) # (n_queries, k, in_channels*2)
            else:
                x = x - q_points.unsqueeze(1)
            ...
            x[out_ranged, :] = torch_zeros_like(x[:1, :])
        ...
        if 'feat2' in self.feat_mode and not self.strided:
            ...
            y = torch_cat((y-y_center, y_center), dim=2) # (n_queries, k, feat_channels*2)

        # feat+points
        if 'joint' in self.feat_mode:
            y = torch_cat((x, y), dim=2)
        ...
        x = torch_matmul(kernel, x).squeeze(4) # (bs, n_queries, k, out_channels)
...
class AdaptResnetBottleneckBlock(torch_nn_Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        if in_dim != out_dim:
            ...
            self.unary_shortcut = torch_nn_Identity()

        # Other operations
        self.leaky_relu = torch_nn_LeakyReLU(0.1)
...
class AdaptSimpleBlock(torch_nn_Module):
...
# class GraphConv(torch_nn_Module):
...
#         self.conv = torch_nn_Sequential(
#             torch_nn_Conv2d(self.feat_channels, self.out_channels, kernel_size=1, bias=False),
#             torch_nn_BatchNorm2d(self.out_channels),
#             torch_nn_LeakyReLU(negative_slope=0.1))
...
# class GraphResnetBottleneckBlock(torch_nn_Module):
...
# class GraphSimpleBlock(torch_nn_Module):
...
class BatchNormBlock(torch_nn_Module):
    ...
    def __init__(self, in_dim, use_bn, bn_momentum):
        ...
        self.in_dim = in_dim
        if self.use_bn:
            self.batch_norm = torch_nn_BatchNorm1d(in_dim, momentum=bn_momentum)
            #self.batch_norm = torch_nn_InstanceNorm1d(in_dim, momentum=bn_momentum)
        else:
            self.bias = Parameter(torch_zeros(in_dim, dtype=torch_float32), requires_grad=True)
        return

    # def reset_parameters(self):
    #     torch_nn_init.zeros_(self.bias)
...
class UnaryBlock(torch_nn_Module):
    ...
    def __init__(self, in_dim, out_dim, use_bn, bn_momentum, no_relu=False):
        ...
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.mlp = torch_nn_Linear(in_dim, out_dim, bias=False)
...
# class GlobalAverageBlock(torch_nn_Module):
...
class NearestUpsampleBlock(torch_nn_Module):
...
# class MaxPoolBlock(torch_nn_Module):
...
if __name__ == '__main__':
    ...
    batch = TempBatch()
    batch.points = [torch_rand(npoints1, 3)]
    batch.points.append(torch_rand(npoints2, 3))
    batch.neighbors = [torch_randint(0, npoints1, (npoints1, n1))]
    batch.pools = [torch_randint(0, npoints1, (npoints2, n2))]
    feature = torch_rand(npoints1, in_dim)
